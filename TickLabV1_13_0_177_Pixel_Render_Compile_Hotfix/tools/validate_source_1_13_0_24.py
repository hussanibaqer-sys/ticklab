...
for token, label in [
    ('<Version>1.13.0.24</Version>', 'project version'),
    ('<AssemblyVersion>1.13.0.24</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.24</FileVersion>', 'file version'),
]:
    check(token in project, label)
check((ROOT / 'TickLabV1_13_0_24.sln').exists(), 'solution file')
check(text(ROOT / 'VERSION.txt').strip() == '1.13.0.24', 'VERSION.txt')
check('1.13.0.24' in main_xaml, 'main window version title')
...
# V1.13.0.24 basic candle right-edge zoom stability checks plus v1.13.0.22 regressions.
...
print(f'V1.13.0.24 CHECKS PASSED: {passed}')
print(f'V1.13.0.24 CHECKS FAILED: {len(failures)}')
if failures:
    for failure in failures[:150]: print('FAIL:', failure)
    raise SystemExit(1)
print('TickLab v1.13.0.24 basic candle right-edge zoom validation passed.')
