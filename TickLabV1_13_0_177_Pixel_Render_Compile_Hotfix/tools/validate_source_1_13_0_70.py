...
mt5_history = read(app / 'Windows' / 'Mt5TradeHistoryService.cs')
trade_models = read(app / 'Windows' / 'TradeHistoryModels.cs')
trade_report = read(app / 'Windows' / 'TradeReportWindow.cs')

# Release identity.
for needle, label in [
    ('<Version>1.13.0.70</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.70</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.70</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_70.sln').exists(), 'v55 solution exists')
check(not (root / 'TickLabV1_13_0_54.sln').exists(), 'old v54 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.70', 'VERSION file')
check('TickLabV1_13_0_70.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v55')
check('Drawing Toolbar Parity Overhaul' in xaml, 'window title')
...
check('Content="Project demo history on chart"' in xaml, 'history toggle label')
...
check('DemoSymbolsMatch(item.Symbol, context.Symbol)' in demo and 'TradeOverlapsProjectionWindow' in demo,
      'history remains symbol scoped and viewport bounded across charts/timeframes')
...
# v1.13.0.70 targeted regression and integrity checks
...
# v1.13.0.70 narrow hotfix checks.
...
# v1.13.0.70 atomic history/live merge checks.
...
# v1.13.0.70 source/display aliasing regression checks.
...
# v1.13.0.70 tick replay and vertical-line regression checks.
...
# v1.13.0.70 replay selector and range workflow.
...
# v1.13.0.70 instant replay performance and hidden-live checks.
...
# v1.13.0.70 compact Replay controls and visible speed checks.
...
# v1.13.0.70 replay speed, compact default, selector sharpness/colour and alert colour.
...
# v1.13.0.70 immediate range completion, selector pixels and alert bulk management.
...
# v1.13.0.70 raw Tick chart and alert-contrast checks.
...
check(not (root / 'TickLabV1_13_0_62.sln').exists() and (root / 'TickLabV1_13_0_70.sln').exists(),
...
# v1.13.0.70 Tick chart-type, scale parity, replay layering/performance and turbo checks.
...
# v1.13.0.70 recorder, right-space and ultra-speed checks.
...
# v1.13.0.70 recorder regression: chart-priority capture and fast-player AVI metadata.
...
# v1.13.0.70 dedicated screenshot button and collapsed PANEL startup.
recorder_main = read(app / 'MainWindow.ScreenRecorder.cs')
check('x:Name="ScreenshotButton"' in xaml and 'Click="ScreenshotButton_Click"' in xaml,
      'top bar has dedicated screenshot button beside recorder')
check('private void ScreenshotButton_Click' in recorder_main and 'PngBitmapEncoder' in recorder_main and
      'ScreenRecorderWindow.RecordingsFolder' in recorder_main,
      'top-bar screenshot saves PNG into shared TickLab Recordings folder')
check('RecordingMetadata.Save(path, string.Empty, "Screenshot")' in recorder_main,
      'top-bar screenshot writes recording metadata')
check('RefreshMediaList()' in read(app / 'Windows' / 'ScreenRecorderWindow.cs') and
      '_screenRecorderWindow?.RefreshMediaList();' in recorder_main,
      'open recorder list refreshes after top-bar screenshot')
check('private bool _rightWorkspaceCollapsed = true;' in main and
      '_rightWorkspaceCollapsed = true;' in main and 'ApplyRightWorkspaceCollapsedState();' in main,
      'main PANEL explicitly starts collapsed in code')
check('x:Name="RightWorkspaceColumn" Width="0"' in xaml and
      'x:Name="RightWorkspaceBorder"' in xaml and 'Visibility="Collapsed"' in xaml,
      'main PANEL is collapsed in initial XAML before first frame')
check('TickLabV1_13_0_70.sln' in read(root / 'Clean-Restore-Build.cmd'),
      'build script targets v67 solution')

# v68 MT5 history library, projection and full reports.
check('Header="MT5 trade history"' in xaml, 'MT5 trade history tab present')
check('Content="Open Folder"' in xaml and 'DemoMt5HistoryOpenFolderButton_Click' in demo, 'MT5 history Open Folder present')
check('Content="Project on chart"' in xaml and 'DemoMt5HistoryProjectionCheckBox_Changed' in demo, 'per-file MT5 projection checkbox present')
check('DemoMt5HistoryOpenReportButton_Click' in demo and 'Content="Open Report"' in xaml, 'history report buttons present')
check('Project demo history on chart' in xaml, 'demo projection control moved into trade history')
check('Mt5TradeHistoryService.HistoryFolder' in demo and 'Environment.SpecialFolder.MyDocuments' in mt5_history, 'user MT5 history folder')
for ext in ['.html', '.htm', '.csv', '.txt', '.tsv']:
    check(ext in mt5_history, f'MT5 parser supports {ext}')
check('ExtractHtmlRows' in mt5_history and 'ExtractTextRows' in mt5_history, 'HTML and copied text parsing')
check('PairDealEvents' in mt5_history and 'RemainingVolume' in mt5_history, 'MT5 deal IN/OUT pairing')
check('AppendProjectedMt5HistoryLines' in demo and 'DemoTradeLineKind.HistoryEntry' in demo and 'DemoTradeLineKind.HistoryExit' in demo, 'MT5 entry/exit chart projection')
check('SaveProjectedPaths' in mt5_history and 'LoadProjectedPaths' in mt5_history, 'MT5 projection state persistence')
for label in ['Starting balance', 'Deposits', 'Withdrawals', 'Ending balance', 'Net P/L', 'Win rate', 'Profit factor', 'Max drawdown', 'Commission', 'Swap', 'Fees']:
    check(label in trade_report, f'report metric {label}')
for graph in ['Balance / realized equity', 'Drawdown', 'Daily profit / loss', 'Trades per day', 'Net profit by symbol', 'Monthly profit / loss', 'Trade-by-trade net P/L']:
    check(graph in trade_report, f'report graph {graph}')
check('Zoom out' in trade_report and 'Zoom in' in trade_report and 'Reset zoom' in trade_report and 'ModifierKeys.Control' in trade_report, 'report zoom controls and Ctrl-wheel zoom')
check('HorizontalScrollBarVisibility = ScrollBarVisibility.Auto' in trade_report and 'VerticalScrollBarVisibility = ScrollBarVisibility.Auto' in trade_report, 'report scrollable')
check('CanUserSortColumns = true' in trade_report and 'view.Filter' in trade_report, 'report trade table sortable and searchable')
check('TradeHistoryCashFlow' in trade_models and 'BalanceAfter' in trade_models, 'report cash-flow/balance models')

# v1.13.0.70 viewport-bounded history projection and modern multi-page report.
check('ScheduleHistoryProjectionRefresh' in demo and 'DispatcherPriority.Background' in demo,
      'history projection refresh is coalesced behind chart interaction')
check('TryGetHistoryProjectionWindow' in demo and 'CaptureViewportSnapshot' in demo and
      'TradeOverlapsProjectionWindow' in demo,
      'projected history is bounded to visible chart time plus buffer')
check('GetProjectionTrades(historyWindowStart, historyWindowEnd)' in demo and
      'UpperBoundOpenUnix' in trade_models and 'LowerBoundPrefixClose' in trade_models and
      'PrefixMaxCloseUnix' in trade_models,
      'MT5 history projection uses timestamp index instead of rescanning the whole report')
check('denseProjection = visible.Length > 600' in demo and '!denseProjection && trade.StopLoss > 0' in demo,
      'dense MT5 projection preserves trades while suppressing expensive auxiliary SL TP lines')
check('denseHistory = pairs.Length > 600' in chart_demo and 'hoverGroups.Length > 1200' in chart_demo,
      'dense renderer suppresses captions and expensive hover scans at extreme density')
for page in ['Summary', 'Profit & Loss', 'Long & Short', 'Symbols', 'Risks', 'Trades']:
    check(f'AddPage("{page}"' in trade_report, f'modern report page {page}')
check('ApplicationThemeManager.ApplyToWindow(this)' in trade_report and
      'SetResourceReference(BackgroundProperty, "WindowBrush")' in trade_report and
      'ResourceBrush("PanelBrush"' in trade_report,
      'report follows TickLab light dark resources including custom graphs')
check('DonutReportControl' in trade_report and 'DirectionDonutControl' in trade_report and
      'AddPercentMeter' in trade_report,
      'report includes modern percentage and donut visualizations')
check('MouseMove += OnMouseMove' in trade_report and 'ToolTip = $"{point.Label}' in trade_report,
      'report performance graphs provide interactive hover values')

report = root / 'VALIDATION_REPORT_1_13_0_69.txt'
report.write_text(
    'TickLab v1.13.0.70 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
...
# v70 Drawing Toolbar Parity Overhaul focused validation.
drawing_models = read(app / 'Core' / 'Drawing' / 'DrawingModels.cs')
drawing_catalog = read(app / 'Core' / 'Drawing' / 'DrawingToolCatalog.cs')
drawing_defaults = read(app / 'Core' / 'Drawing' / 'DrawingParityDefaults.cs')
drawing_icons = read(app / 'Core' / 'Drawing' / 'DrawingToolIconFactory.cs')
drawing_engine = read(app / 'Controls' / 'CandleChartControl.Drawing.cs')
drawing_parity = read(app / 'Controls' / 'CandleChartControl.DrawingParity.cs')
drawing_settings_xaml = read(app / 'Windows' / 'DrawingSettingsWindow.xaml')
drawing_settings_code = read(app / 'Windows' / 'DrawingSettingsWindow.xaml.cs')
drawing_favorites_xaml = read(app / 'Windows' / 'DrawingFavoritesWindow.xaml')
drawing_palette_xaml = read(app / 'Windows' / 'DrawingToolPaletteWindow.xaml')
app_xaml = read(app / 'App.xaml')

check((app / 'Core' / 'Drawing' / 'DrawingParityDefaults.cs').exists(), 'parity defaults source exists')
check((app / 'Controls' / 'CandleChartControl.DrawingParity.cs').exists(), 'specialized parity renderer source exists')
check('DrawTradingViewParityDrawing' in drawing_engine and 'DrawTradingViewParityDrawing' in drawing_parity,
      'specialized drawing renderer invoked before generic renderer')

# Official-style group/catalog coverage and independent advanced tools.
for tool_id in [
    'cursor-crosshair','cursor-dot','cursor-arrow','cursor-demo','cursor-magic','eraser',
    ...
    'fib-retracement','trend-fib-extension','pitchfork','schiff-pitchfork','modified-schiff-pitchfork','inside-pitchfork',
    'fib-channel','fib-time-zone','gann-box','gann-square','gann-fan','fib-speed-fan','trend-fib-time','fib-circles',
    'pitchfan','fib-spiral','fib-speed-arcs','fib-wedge',
    'rectangle','rotated-rectangle','path','circle','ellipse','polyline','triangle','arc','curve','double-curve','brush','highlighter',
    'text','note','price-note','pin','table','callout','comment','price-label','signpost','flag-mark',
    'xabcd-pattern','abcd-pattern','triangle-pattern','three-drives-pattern','head-shoulders','elliott-impulse','elliott-triangle',
    'elliott-triple-combo','elliott-correction','elliott-double-combo','cyclic-lines','time-cycles','sine-line',
    'long-position','short-position','position-forecast','bars-pattern','ghost-feed','sector','anchored-vwap',
    'fixed-volume-profile','anchored-volume-profile','price-range','date-range','date-price-range']:
    check(f'Add("{tool_id}"' in drawing_catalog, f'drawing catalog {tool_id}')

check('case "trend-fib-extension"' in drawing_parity and 'points.Length >= 3' in drawing_parity and
      'DrawParityFibExtension' in drawing_parity, 'trend fib extension uses dedicated 3-anchor renderer')
check('case "trend-fib-time"' in drawing_parity and 'DrawParityTrendFibTime' in drawing_parity,
      'trend fib time uses dedicated 3-anchor renderer')
for needle,label in [
    ('case "schiff-pitchfork"', 'Schiff pitchfork variant'),
    ('case "modified-schiff-pitchfork"', 'Modified Schiff pitchfork variant'),
    ('case "inside-pitchfork"', 'Inside pitchfork variant'),
    ('"schiff-pitchfork" => Mid(a, b)', 'pitchfork variant geometry branch')]:
    check(needle in drawing_parity, label)
check('PearsonsR' in drawing_defaults and 'UpperDeviation' in drawing_defaults and 'LowerDeviation' in drawing_defaults and
      'Source' in drawing_defaults, 'regression configurable source/deviations/Pearson R')
check('ParitySource' in drawing_parity and 'pearson' in drawing_parity.lower(), 'regression source and Pearson rendered')
check('Band1Multiplier' in drawing_defaults and 'Band2Multiplier' in drawing_defaults and 'Band3Multiplier' in drawing_defaults,
      'anchored VWAP three band settings')
check('bandLevels' in drawing_parity and 'ParityLevelPen(drawing, level' in drawing_parity and 'Band #3' in drawing_defaults,
      'anchored VWAP uses independent band level styling')
check('DrawParityCircle' in drawing_parity and 'dc.DrawEllipse' in drawing_parity, 'true circle dedicated renderer')
check('DrawParityPath' in drawing_parity and 'BezierTo' in drawing_parity, 'path has smooth vector geometry')
check('ratio' in drawing_parity.lower() and 'DrawParityPattern' in drawing_parity, 'pattern tools render leg ratios/guides')
check('AccountSize' in drawing_defaults and 'Risk' in drawing_defaults and 'RiskIsPercent' in drawing_defaults and 'Leverage' in drawing_defaults and
      'LotSize' in drawing_defaults, 'position tool account/risk/lot/leverage settings')
check('qtyRisk' in drawing_parity and 'riskMoney' in drawing_parity and 'targetBalance' in drawing_parity, 'position tool calculates quantity and risk')
check('c.High' in drawing_parity and 'c.Low' in drawing_parity and 'DrawParityBarsPattern' in drawing_parity,
      'bars pattern projects full OHLC geometry')
check('ValueAreaPercent' in drawing_defaults and 'ShowPOC' in drawing_defaults and 'UpDownVolume' in drawing_defaults,
      'volume profile value area POC up/down controls')
check('valueLow' in drawing_parity and 'valueHigh' in drawing_parity and 'poc' in drawing_parity,
      'volume profile calculates value area and POC')
check('case "pin"' in drawing_parity and 'DrawParityPin' in drawing_parity, 'Pin has dedicated renderer')
check('case "table"' in drawing_parity and 'DrawParityTable' in drawing_parity, 'Table has dedicated renderer')
check('case "sector"' in drawing_parity and 'ShowAngle' in drawing_defaults and 'ShowRadius' in drawing_defaults,
      'Sector angle/radius behavior')
check('case "cyclic-lines"' in drawing_parity and 'Count' in drawing_defaults, 'cycle count setting')
check('case "sine-line"' in drawing_parity and 'Cycles' in drawing_defaults, 'sine cycles setting')

# Exact style/settings behavior.
for needle,label in [
    ('HorizontalTextAlignment', 'horizontal text alignment'),
    ('VerticalTextAlignment', 'vertical text alignment'),
    ('MinimumTimeframe', 'minimum interval visibility'),
    ('MaximumTimeframe', 'maximum interval visibility'),
    ('DrawingSyncMode', 'per-drawing sync mode'),
    ('ShowPriceLabels', 'price label setting'),
    ('ShowTimeLabels', 'time label setting'),
    ('ShowStatistics', 'statistics setting')]:
    check(needle in drawing_models or needle in drawing_engine or needle in drawing_settings_code,
          f'drawing setting {label}')
check('DrawingTimeframeSeconds' in drawing_engine and 'MinimumTimeframe' in drawing_engine and 'MaximumTimeframe' in drawing_engine,
      'exact min/max timeframe visibility enforced')
check('HorizontalAlignmentBox' in drawing_settings_xaml and 'VerticalAlignmentBox' in drawing_settings_xaml and
      'HorizontalTextAlignment' in drawing_settings_code and 'VerticalTextAlignment' in drawing_settings_code,
      'text alignment controls exposed')
check('MinimumTimeframeBox' in drawing_settings_xaml and 'MaximumTimeframeBox' in drawing_settings_xaml,
      'exact interval visibility controls exposed')
check('Templates' in drawing_settings_xaml and 'Coordinates' in drawing_settings_xaml and 'Levels' in drawing_settings_xaml,
      'settings window exposes templates coordinates levels')
check('DrawingParityDefaults.MergeOptions' in drawing_settings_code, 'templates merge new tool-specific defaults safely')

# Synchronization must respect scope and source ownership rather than duplicating all drawings everywhere.
check('drawing.SyncMode switch' in contexts, 'drawing synchronization branches by SyncMode')
for mode in ['CurrentChart','SameSymbol','SameSymbolAndTimeframe','CurrentLayout','Global']:
    check(mode in contexts and mode in main, f'sync mode {mode} wired')
check('string.Equals(drawing.ChartId, source.DrawingOwnerId' in contexts,
      'sync only republishes source-owned drawings')
check('ReplaceSynchronizedDrawings' in contexts and 'ReplaceSynchronizedDrawings' in drawing_engine,
      'sync replaces remote copies while preserving local drawings')
check('DefaultDrawingSyncMode' in drawing_engine and 'SetDefaultDrawingSyncMode' in main,
      'new drawings inherit toolbar sync mode')

# Left rail: official utility controls and crisp TickLab-owned vector rendering.
for control in ['DrawingMeasureButton','DrawingZoomButton','DrawingMagnetButton','StayDrawingModeButton','DrawingLockButton',
                'DrawingHideButton','DrawingObjectTreeButton','DrawingSyncButton','DrawingUndoButton','DrawingRedoButton','DrawingRemoveButton']:
    check(f'x:Name="{control}"' in xaml, f'left rail control {control}')
check('DrawingBrushButton' in xaml and 'Visibility="Collapsed"' in xaml, 'obsolete duplicate Brush rail button hidden')
check('EdgeMode.Aliased' in drawing_icons and 'BitmapScalingMode.NearestNeighbor' in drawing_icons,
      'drawing icons request crisp aliased/nearest-neighbor rendering')
check('SnapsToDevicePixels="True"' in xaml and 'UseLayoutRounding="True"' in xaml,
      'left workspace pixel snapping/layout rounding')
check('<Setter Property="Effect" Value="{x:Null}"/>' in app_xaml and 'x:Key="DrawingRailButton"' in app_xaml,
      'drawing rail button style disables effects')
for name,text in [('MainWindow.xaml',xaml),('DrawingSettingsWindow.xaml',drawing_settings_xaml),
                  ('DrawingFavoritesWindow.xaml',drawing_favorites_xaml),('DrawingToolPaletteWindow.xaml',drawing_palette_xaml)]:
    check('BlurEffect' not in text and 'DropShadowEffect' not in text,
          f'no blur/shadow effect in drawing UI {name}')

# Cursor modes render distinctly.
check('cursor-dot' in chart and 'cursor-demo' in chart and 'cursor-magic' in chart and 'cursor-arrow' in chart,
      'distinct cursor modes handled in chart renderer')
check('DrawEllipse' in chart and 'DrawLine' in chart, 'cursor dot/cross visuals use vector primitives')

check('Add("Mode", 0)' in drawing_defaults and 'mode switch' in drawing_parity,
      'Bars Pattern supports bar-stick and source-line modes')
check('DrawingUiBrush("TextBrush"' in main and 'DrawingUiBrush("AccentSoftBrush"' in main,
      'drawing rail and palettes follow TickLab light/dark theme brushes')
# Current v70 targeted parity checks should materially extend inherited coverage.
