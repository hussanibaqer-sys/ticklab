...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.48</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.48</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.48</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_48.sln').exists(), 'v48 solution exists')
check(not (root / 'TickLabV1_13_0_46.sln').exists(), 'old v46 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.48', 'VERSION file')
check('TickLabV1_13_0_48.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v48')
check('Chart Integrity, Alerts and Rail Stability' in xaml, 'window title')
...
# v1.13.0.48 targeted regression and integrity checks
...
report = root / 'VALIDATION_REPORT_1_13_0_48.txt'
report.write_text(
    'TickLab v1.13.0.48 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
