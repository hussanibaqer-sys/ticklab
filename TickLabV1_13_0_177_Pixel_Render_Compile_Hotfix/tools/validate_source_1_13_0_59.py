...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.59</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.59</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.59</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_59.sln').exists(), 'v55 solution exists')
check(not (root / 'TickLabV1_13_0_54.sln').exists(), 'old v54 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.59', 'VERSION file')
check('TickLabV1_13_0_59.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v55')
check('Compact Replay Controls' in xaml, 'window title')
...
# v1.13.0.59 targeted regression and integrity checks
...
# v1.13.0.59 narrow hotfix checks.
...
# v1.13.0.59 atomic history/live merge checks.
...
# v1.13.0.59 source/display aliasing regression checks.
...
# v1.13.0.59 tick replay and vertical-line regression checks.
...
# v1.13.0.59 replay selector and range workflow.
...
# v1.13.0.59 instant replay performance and hidden-live checks.
...
# v1.13.0.59 compact Replay controls and visible speed checks.
...
report = root / 'VALIDATION_REPORT_1_13_0_59.txt'
report.write_text(
    'TickLab v1.13.0.59 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
