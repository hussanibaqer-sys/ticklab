...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.47</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.47</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.47</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_47.sln').exists(), 'v47 solution exists')
check(not (root / 'TickLabV1_13_0_46.sln').exists(), 'old v46 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.47', 'VERSION file')
check('TickLabV1_13_0_47.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v47')
check('Trade History, Alerts and Symbol Favourites' in xaml, 'window title')
...
check('AlertBellPlayer.PlayOnce();' in alerts and alerts.index('AlertBellPlayer.PlayOnce();') < alerts.index('if (rule.ShowDesktopPopup'),
      'bell starts immediately before popup')
...
check('Color exitColor = Color.FromRgb(224, 75, 90);' in chart_demo,
      'history exit is red')
check('CreateText(caption, 4.5, Brushes.White)' in chart_demo,
      'history caption text is about half size')
...
report = root / 'VALIDATION_REPORT_1_13_0_47.txt'
report.write_text(
    'TickLab v1.13.0.47 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
