...
for token,label in [
('<Version>1.13.0.18</Version>','project version'),
('<AssemblyVersion>1.13.0.18</AssemblyVersion>','assembly version'),
('<FileVersion>1.13.0.18</FileVersion>','file version'),
]: check(token in project,label)
check((ROOT/'TickLabV1_13_0_18.sln').exists(),'solution file')
check(text(ROOT/'VERSION.txt').strip()=='1.13.0.18','VERSION.txt')
check('400-Bar Full Candle Zoom Repair' in main,'window title')
...
check('_detailedCandlePitchPixels < 3' in chart,'crisp plot supports 3px pitch')
...
# Geometry model matching v1.13.0.18.
...
print(f'V1.13.0.18 CHECKS PASSED: {checks-len(failures)}')
print(f'V1.13.0.18 CHECKS FAILED: {len(failures)}')
if failures:
    for f in failures[:100]: print('FAIL:',f)
    raise SystemExit(1)
print('TickLab v1.13.0.18 400-bar full candle zoom validation passed.')
