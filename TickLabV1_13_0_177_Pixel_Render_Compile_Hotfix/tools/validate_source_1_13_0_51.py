...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.51</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.51</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.51</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_51.sln').exists(), 'v51 solution exists')
check(not (root / 'TickLabV1_13_0_48.sln').exists(), 'old v48 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.51', 'VERSION file')
check('TickLabV1_13_0_51.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v51')
check('Candle List Aliasing Hotfix' in xaml, 'window title')
...
# v1.13.0.51 targeted regression and integrity checks
...
# v1.13.0.51 narrow hotfix checks.
...
# v1.13.0.51 atomic history/live merge checks.
...
# v1.13.0.51 source/display aliasing regression checks.
...
report = root / 'VALIDATION_REPORT_1_13_0_51.txt'
report.write_text(
    'TickLab v1.13.0.51 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
