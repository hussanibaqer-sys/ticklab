...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.49</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.49</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.49</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_49.sln').exists(), 'v49 solution exists')
check(not (root / 'TickLabV1_13_0_48.sln').exists(), 'old v48 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.49', 'VERSION file')
check('TickLabV1_13_0_49.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v49')
check('Full History and Alert Text Hotfix' in xaml, 'window title')
...
# v1.13.0.49 targeted regression and integrity checks
...
# v1.13.0.49 narrow hotfix checks.
...
report = root / 'VALIDATION_REPORT_1_13_0_49.txt'
report.write_text(
    'TickLab v1.13.0.49 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
