...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.54</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.54</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.54</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_54.sln').exists(), 'v53 solution exists')
check(not (root / 'TickLabV1_13_0_52.sln').exists(), 'old v52 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.54', 'VERSION file')
check('TickLabV1_13_0_54.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v53')
check('Replay Range Selectors' in xaml, 'window title')
...
# v1.13.0.54 targeted regression and integrity checks
...
# v1.13.0.54 narrow hotfix checks.
...
# v1.13.0.54 atomic history/live merge checks.
...
# v1.13.0.54 source/display aliasing regression checks.
...
# v1.13.0.54 tick replay and vertical-line regression checks.
...
# v1.13.0.54 replay selector and range workflow.
...
check('EndUnixExclusive' in alerts and
      'endMilliseconds: endUnixExclusive.HasValue ? checked(endUnixExclusive.Value * 1000L) : null' in alerts and
...
check('context.Chart.InteractiveReplayEndMarker = _replayRangeMode ? replayEndMarker : null;' in alerts and
...
report = root / 'VALIDATION_REPORT_1_13_0_54.txt'
report.write_text(
    'TickLab v1.13.0.54 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
