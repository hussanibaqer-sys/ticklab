...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.58</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.58</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.58</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_58.sln').exists(), 'v55 solution exists')
check(not (root / 'TickLabV1_13_0_54.sln').exists(), 'old v54 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.58', 'VERSION file')
check('TickLabV1_13_0_58.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v55')
check('Replay Controls + Viewport Fix' in xaml, 'window title')
...
# v1.13.0.58 targeted regression and integrity checks
...
# v1.13.0.58 narrow hotfix checks.
...
# v1.13.0.58 atomic history/live merge checks.
...
# v1.13.0.58 source/display aliasing regression checks.
...
# v1.13.0.58 tick replay and vertical-line regression checks.
...
# v1.13.0.58 replay selector and range workflow.
...
# v1.13.0.58 instant replay performance and hidden-live checks.
...
report = root / 'VALIDATION_REPORT_1_13_0_58.txt'
report.write_text(
    'TickLab v1.13.0.58 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
