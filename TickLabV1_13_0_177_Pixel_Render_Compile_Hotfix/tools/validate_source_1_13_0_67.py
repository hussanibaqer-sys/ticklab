...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.67</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.67</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.67</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_67.sln').exists(), 'v55 solution exists')
check(not (root / 'TickLabV1_13_0_54.sln').exists(), 'old v54 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.67', 'VERSION file')
check('TickLabV1_13_0_67.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v55')
check('Screenshot Button + Collapsed Panel Launch' in xaml, 'window title')
...
# v1.13.0.67 targeted regression and integrity checks
...
# v1.13.0.67 narrow hotfix checks.
...
# v1.13.0.67 atomic history/live merge checks.
...
# v1.13.0.67 source/display aliasing regression checks.
...
# v1.13.0.67 tick replay and vertical-line regression checks.
...
# v1.13.0.67 replay selector and range workflow.
...
# v1.13.0.67 instant replay performance and hidden-live checks.
...
# v1.13.0.67 compact Replay controls and visible speed checks.
...
# v1.13.0.67 replay speed, compact default, selector sharpness/colour and alert colour.
...
# v1.13.0.67 immediate range completion, selector pixels and alert bulk management.
...
# v1.13.0.67 raw Tick chart and alert-contrast checks.
...
check(not (root / 'TickLabV1_13_0_62.sln').exists() and (root / 'TickLabV1_13_0_67.sln').exists(),
...
# v1.13.0.67 Tick chart-type, scale parity, replay layering/performance and turbo checks.
...
# v1.13.0.67 recorder, right-space and ultra-speed checks.
...
# v1.13.0.67 recorder regression: chart-priority capture and fast-player AVI metadata.
...
# v1.13.0.67 dedicated screenshot button and collapsed PANEL startup.
recorder_main = read(app / 'MainWindow.ScreenRecorder.cs')
...
check('TickLabV1_13_0_67.sln' in read(root / 'Clean-Restore-Build.cmd'),
      'build script targets v67 solution')

report = root / 'VALIDATION_REPORT_1_13_0_67.txt'
report.write_text(
    'TickLab v1.13.0.67 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
