...
for token, label in [
    ('<Version>1.13.0.21</Version>', 'project version'),
    ('<AssemblyVersion>1.13.0.21</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.21</FileVersion>', 'file version'),
]:
    check(token in project, label)
check((ROOT / 'TickLabV1_13_0_21.sln').exists(), 'solution file')
check(text(ROOT / 'VERSION.txt').strip() == '1.13.0.21', 'VERSION.txt')
check('MT5 Indicators + Stable All-Chart Zoom' in main_xaml, 'main window title')
...
print(f'V1.13.0.21 CHECKS PASSED: {passed}')
print(f'V1.13.0.21 CHECKS FAILED: {len(failures)}')
if failures:
    for failure in failures[:150]: print('FAIL:', failure)
    raise SystemExit(1)
print('TickLab v1.13.0.21 indicator and all-chart zoom validation passed.')
