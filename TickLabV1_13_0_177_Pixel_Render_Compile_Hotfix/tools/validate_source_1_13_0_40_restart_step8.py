...
base=Path('/mnt/data/work_step8/TickLabV1_13_0_39_Restart_Step7_Demo_Trading_Reliability_History')
...
# Release identity.
for needle,label in [('<Version>1.13.0.40</Version>','version'),('<AssemblyVersion>1.13.0.40</AssemblyVersion>','assembly version'),('<FileVersion>1.13.0.40</FileVersion>','file version')]: check(needle in proj,label)
check((root/'TickLabV1_13_0_40.sln').exists(),'solution exists')
check((root/'VERSION.txt').read_text().strip()=='1.13.0.40','VERSION file')
check('TickLabV1_13_0_40.sln' in read('Clean-Restore-Build.cmd'),'clean build script targets Step 8 solution')
check('Restart Step 8 Trade Level Labels and UI Contrast' in xaml,'window title')
...
for needle in ['DrawAskPriceLine(drawingContext, layout)','private void DrawAskPriceLine','askPrice.ToString($"F{digits}"','live.Close + Math.Max(0, live.Spread) * point']:
...
for needle,label in [
    ...
    ('<Style TargetType="Button" BasedOn="{StaticResource {x:Type Button}}">','demo scoped readable button style'),
...
# Step 8 active-trade labels, Ask ticket, right workspace handle and contrast fixes.
for needle,label in [
    ('CURRENT {FormatDemoUsdAmount(position.FloatingProfit)}','entry line shows live USD amount'),
    ('CalculateDemoProfitAtPrice(position, position.StopLoss)','SL line shows projected USD amount'),
    ('CalculateDemoProfitAtPrice(position, position.TakeProfit)','TP line shows projected USD amount'),
    ('private static double CalculateDemoProfitAtPrice','shared projected P/L helper'),
    ('private static string FormatDemoUsdAmount','USD label formatter'),
    ('if (_demoOpenPositions.Count > 0)\n            RefreshDemoTradeLines();','active level labels refresh with live market'),
]: check(needle in demo,label)
check('ASK {askPrice' not in chart,'Ask line no longer writes ASK')
check('askPrice.ToString($"F{digits}", CultureInfo.InvariantCulture)' in chart,'Ask line ticket shows price only')
for needle,label in [
    ('RightWorkspaceHandle_LostMouseCapture','middle slide handle recovers lost mouse capture'),
    ('RightWorkspaceHandle_PreviewKeyDown','middle slide handle keyboard toggle'),
    ('FinishRightWorkspaceHandleInteraction','middle slide interaction completes safely'),
    ('Mouse.Capture(handle, CaptureMode.Element)','middle slide handle captures mouse'),
    ('SaveWorkspace();','middle slide state persists'),
]: check(needle in all_main+xaml,label)
for needle,label in [
    ('DemoPresetButtonStyle','SL/TP preset scoped contrast style'),
    ('TextElement.Foreground="{TemplateBinding Foreground}"','preset text follows readable foreground'),
    ('DemoLightComboBoxStyle','pending selectors light contrast style'),
    ('DemoLightComboBoxItemStyle','pending dropdown item contrast style'),
    ('Style="{StaticResource DemoLightComboBoxStyle}"','pending Type/Expiration use readable style'),
    ('<Setter Property="Background" Value="#2B5685"/>','selected dropdown state visible'),
    ('<Setter Property="Foreground" Value="White"/>','selected dropdown text readable'),
]: check(needle in xaml,label)
check(xaml.count('Style="{StaticResource DemoPresetButtonStyle}"') == 11,'all eleven SL/TP preset buttons use contrast style')
check(xaml.count('Style="{StaticResource DemoLightComboBoxStyle}"') == 2,'Type and Expiration selectors both use contrast style')
...
print(f'STEP8 CHECKS PASSED: {len(passed)}')
print(f'STEP8 CHECKS FAILED: {len(failed)}')
