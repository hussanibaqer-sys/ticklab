...
required=['TickLabV1_13_0_10.sln','RELEASE_NOTES_1_13_0_10.txt','BUILD_FIX_README_1_13_0_10.txt','VERSION.txt','README.txt','DRAWING_TOOLS_GUIDE.txt','BRIDGE_SETUP.txt','FIRST_TEST_CHECKLIST.txt','LIVE_COMPATIBILITY_AUDIT.txt','MT5_SOURCE_SHA256.txt','Clean-Restore-Build.cmd']
for n in required: check((ROOT/n).is_file(),f'package file exists: {n}')
project=read(APP/'TickLab.App.csproj'); solution=read(ROOT/'TickLabV1_13_0_10.sln'); cmd=read(ROOT/'Clean-Restore-Build.cmd')
check('<Version>1.13.0.10</Version>' in project,'project version is 1.13.0.10')
check('<AssemblyVersion>1.13.0.10</AssemblyVersion>' in project,'assembly version is 1.13.0.10')
check('<FileVersion>1.13.0.10</FileVersion>' in project,'file version is 1.13.0.10')
check('src\\TickLab.App\\TickLab.App.csproj' in solution,'solution references TickLab.App')
check('TickLabV1_13_0_10.sln' in cmd,'build script targets v1.13.0.10')
...
for f,l in features.items(): check(f in mainx,f'v1.13.0.10 shell: {l}')
for f,l in {
...
# v1.13.0.10 natural interaction and media checks
for token,label in {
...
# v1.13.0.10 per-chart appearance, themes and templates
for token,label in {
...
# v1.13.0.10 visual stability regression checks
...
# v1.13.0.10 exact colour mapping and theme isolation
...
if failed:
    ...
    sys.exit(1)
print('All TickLab v1.13.0.10 exact-colour and workspace checks passed.')
