...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.53</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.53</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.53</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_53.sln').exists(), 'v53 solution exists')
check(not (root / 'TickLabV1_13_0_52.sln').exists(), 'old v52 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.53', 'VERSION file')
check('TickLabV1_13_0_53.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v53')
check('Replay Line Checkbox Auto-load Fix' in xaml, 'window title')
...
# v1.13.0.53 targeted regression and integrity checks
...
# v1.13.0.53 narrow hotfix checks.
...
# v1.13.0.53 atomic history/live merge checks.
...
# v1.13.0.53 source/display aliasing regression checks.
...
# v1.13.0.53 tick replay and vertical-line regression checks.
...
check('Cursor = Cursors.SizeWE;' in chart and
      'string.Equals(_interactiveSelectionMarker?.Source, "TickLabReplay"' in chart,
      'replay line exposes drawing-style horizontal drag cursor')
...
check('ProcessNextReplayTick(runtime);' in alerts and
      'RenderReplayChart(forceFit: true);' in alerts,
...
check('move the chart normally away from it' in replay_window and
      'right-click the line to remove it' in replay_window,
      'replay controls explain non-blocking line interaction')

# v1.13.0.53 replay-line checkbox and direct-placement workflow.
check('private readonly CheckBox _replayLineCheckBox;' in replay_window and
      'Content = "Replay line"' in replay_window and
      'ReplayLineChanged?.Invoke(true)' in replay_window and
      'ReplayLineChanged?.Invoke(false)' in replay_window,
      'Replay window has synchronized replay-line checkbox')
check('SetReplayLineChecked(bool enabled)' in replay_window and
      '_synchronizingReplayLineCheckBox' in replay_window,
      'programmatic checkbox synchronization cannot retrigger user events')
check('BeginInteractiveMarkerPlacement' in chart and
      '_interactiveMarkerPlacementPending = true;' in chart and
      'placementPending ||' in chart,
      'checking Replay line arms one-click placement on any visible past candle')
check('InteractiveMarkerPlacementCompleted' in chart and
      'InteractiveMarkerPlacementCompleted?.Invoke(completedMarker);' in chart and
      'chart.InteractiveMarkerPlacementCompleted += CandleChart_InteractiveMarkerPlacementCompleted;' in contexts,
      'marker release is routed once to replay auto-load')
check('marker.StartUnix == long.MinValue' in alerts and
      'await LoadReplayAsync(serverTime);' in alerts,
      'placed replay line automatically loads saved raw ticks')
check('CandleMarker? replayMarker = context.Chart.InteractiveSelectionMarker;' in alerts and
      'StopReplay(restoreChart: true, clearMarker: false);' in alerts and
      'requestedUnix = replayMarker is not null' in alerts,
      'replay loading retains the placed line timestamp instead of stale typed time')
check('context.Chart.InteractiveSelectionMarker = replayMarker;' in alerts and
      'context.Chart.MarkerSelectionMode = true;' in alerts,
      'loaded replay line remains visible and draggable')
check('CandleMarker? retainedReplayMarker = !clearMarker' in alerts and
      'runtime.Context.Chart.InteractiveSelectionMarker = retainedReplayMarker;' in alerts,
      'reloading a moved line preserves it while restoring the pre-replay chart')
check('ClearReplayMarker(bool syncCheckBox = true)' in alerts and
      '_replayWindow?.SetReplayLineChecked(false);' in alerts,
      'right-click remove and untick keep chart line and checkbox synchronized')
check('Click the exact past candle where replay should begin.' in alerts and
      'load saved raw ticks automatically' in alerts,
      'Replay window explains direct placement and automatic loading')

report = root / 'VALIDATION_REPORT_1_13_0_53.txt'
report.write_text(
    'TickLab v1.13.0.53 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
