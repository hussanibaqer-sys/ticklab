...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.69</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.69</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.69</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_69.sln').exists(), 'v55 solution exists')
check(not (root / 'TickLabV1_13_0_54.sln').exists(), 'old v54 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.69', 'VERSION file')
check('TickLabV1_13_0_69.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v55')
check('History Performance + Modern Reports' in xaml, 'window title')
...
# v1.13.0.69 targeted regression and integrity checks
...
# v1.13.0.69 narrow hotfix checks.
...
# v1.13.0.69 atomic history/live merge checks.
...
# v1.13.0.69 source/display aliasing regression checks.
...
# v1.13.0.69 tick replay and vertical-line regression checks.
...
# v1.13.0.69 replay selector and range workflow.
...
# v1.13.0.69 instant replay performance and hidden-live checks.
...
# v1.13.0.69 compact Replay controls and visible speed checks.
...
# v1.13.0.69 replay speed, compact default, selector sharpness/colour and alert colour.
...
# v1.13.0.69 immediate range completion, selector pixels and alert bulk management.
...
# v1.13.0.69 raw Tick chart and alert-contrast checks.
...
check(not (root / 'TickLabV1_13_0_62.sln').exists() and (root / 'TickLabV1_13_0_69.sln').exists(),
...
# v1.13.0.69 Tick chart-type, scale parity, replay layering/performance and turbo checks.
...
# v1.13.0.69 recorder, right-space and ultra-speed checks.
...
# v1.13.0.69 recorder regression: chart-priority capture and fast-player AVI metadata.
...
# v1.13.0.69 dedicated screenshot button and collapsed PANEL startup.
recorder_main = read(app / 'MainWindow.ScreenRecorder.cs')
...
check('TickLabV1_13_0_69.sln' in read(root / 'Clean-Restore-Build.cmd'),
...
# v1.13.0.69 viewport-bounded history projection and modern multi-page report.
...
report.write_text(
    'TickLab v1.13.0.69 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
