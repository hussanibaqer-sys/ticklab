...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.64</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.64</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.64</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_64.sln').exists(), 'v55 solution exists')
check(not (root / 'TickLabV1_13_0_54.sln').exists(), 'old v54 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.64', 'VERSION file')
check('TickLabV1_13_0_64.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v55')
check('Tick Chart Type + Instant Replay Turbo' in xaml, 'window title')
...
# v1.13.0.64 targeted regression and integrity checks
...
# v1.13.0.64 narrow hotfix checks.
...
# v1.13.0.64 atomic history/live merge checks.
...
# v1.13.0.64 source/display aliasing regression checks.
...
# v1.13.0.64 tick replay and vertical-line regression checks.
...
# v1.13.0.64 replay selector and range workflow.
...
# v1.13.0.64 instant replay performance and hidden-live checks.
...
# v1.13.0.64 compact Replay controls and visible speed checks.
...
# v1.13.0.64 replay speed, compact default, selector sharpness/colour and alert colour.
...
# v1.13.0.64 immediate range completion, selector pixels and alert bulk management.
...
# v1.13.0.64 raw Tick chart and alert-contrast checks.
...
check(not (root / 'TickLabV1_13_0_62.sln').exists() and (root / 'TickLabV1_13_0_64.sln').exists(),
...
# v1.13.0.64 Tick chart-type, scale parity, replay layering/performance and turbo checks.
...
check('100×' in replay_window and '250×' in replay_window and '500×' in replay_window and
      ...
      'Math.Clamp(speed, 0.01, 2000.0)' in alerts,
      'Replay exposes requested turbo speeds through 2000x')
check('GetReplayTickBudget' in alerts and '250_000' in alerts and
...
report = root / 'VALIDATION_REPORT_1_13_0_64.txt'
report.write_text(
    'TickLab v1.13.0.64 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
