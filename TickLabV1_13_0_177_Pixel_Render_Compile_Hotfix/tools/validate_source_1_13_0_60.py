...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.60</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.60</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.60</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_60.sln').exists(), 'v55 solution exists')
check(not (root / 'TickLabV1_13_0_54.sln').exists(), 'old v54 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.60', 'VERSION file')
check('TickLabV1_13_0_60.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v55')
check('Replay Labels + Reverse Response' in xaml, 'window title')
...
# v1.13.0.60 targeted regression and integrity checks
...
# v1.13.0.60 narrow hotfix checks.
...
# v1.13.0.60 atomic history/live merge checks.
...
# v1.13.0.60 source/display aliasing regression checks.
...
# v1.13.0.60 tick replay and vertical-line regression checks.
...
# v1.13.0.60 replay selector and range workflow.
...
# v1.13.0.60 instant replay performance and hidden-live checks.
...
# v1.13.0.60 compact Replay controls and visible speed checks.
...
# v1.13.0.60 replay labels, compact End Replay and immediate Reverse response.
...
check('// Give Reverse an immediate visible response.' in alerts and
      'if (ProcessPreviousReplayTick(runtime))' in alerts and
      'RenderReplayChart(forceFit: false);' in alerts,
      'Reverse immediately undoes and redraws one raw tick before timer playback')

report = root / 'VALIDATION_REPORT_1_13_0_60.txt'
report.write_text(
    'TickLab v1.13.0.60 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
