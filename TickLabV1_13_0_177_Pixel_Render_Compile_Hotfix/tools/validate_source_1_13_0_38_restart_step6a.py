...
base=Path('/mnt/data/work_step7/TickLabV1_13_0_37_Restart_Step6')
...
# Release identity.
for needle,label in [('<Version>1.13.0.38</Version>','version'),('<AssemblyVersion>1.13.0.38</AssemblyVersion>','assembly version'),('<FileVersion>1.13.0.38</FileVersion>','file version')]: check(needle in proj,label)
check((root/'TickLabV1_13_0_38.sln').exists(),'solution exists')
check((root/'VERSION.txt').read_text().strip()=='1.13.0.38','VERSION file')
check('TickLabV1_13_0_38.sln' in read('Clean-Restore-Build.cmd'),'clean build script targets Step 6 solution')
check('Restart Step 6A Indicator Context Menu Correction' in xaml,'window title')
...
print(f'STEP6A CHECKS PASSED: {len(passed)}')
print(f'STEP6A CHECKS FAILED: {len(failed)}')
