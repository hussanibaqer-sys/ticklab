...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.55</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.55</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.55</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_55.sln').exists(), 'v55 solution exists')
check(not (root / 'TickLabV1_13_0_54.sln').exists(), 'old v54 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.55', 'VERSION file')
check('TickLabV1_13_0_55.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v55')
check('Instant Replay Index' in xaml, 'window title')
...
# v1.13.0.55 targeted regression and integrity checks
...
# v1.13.0.55 narrow hotfix checks.
...
# v1.13.0.55 atomic history/live merge checks.
...
# v1.13.0.55 source/display aliasing regression checks.
...
# v1.13.0.55 tick replay and vertical-line regression checks.
...
# v1.13.0.55 replay selector and range workflow.
check('private readonly CheckBox _replayLineCheckBox;' in replay_window and
      ...
      'Replay window keeps one synchronized replay-line checkbox')
check('private readonly CheckBox _replayRangeCheckBox;' in replay_window and
      'Content = "Replay range"' in replay_window and
      'ReplayRangeChanged?.Invoke(true)' in replay_window and
      'ReplayRangeChanged?.Invoke(false)' in replay_window,
      'Replay window has explicit replay-range option')
...
# v1.13.0.55 instant replay performance and hidden-live checks.
...
check('Instant Play path: go straight to the small raw-tick source file(s)' in alerts and
      alerts.find('ReadBridgeTicksForReplayFast') < alerts.find('Offline/fallback path'),
      'Play uses indexed raw source before any potentially blocked canonical fallback')
...
check('ScheduleReplayCanonicalWarmup' in alerts and
      'Task background = Task.Run(() => _historyStore.SyncTickArchives' in alerts and
      'Never await this work from the Play button.' in alerts,
      'canonical archive warmup runs only in background after replay begins')
...
check('Width = 620;' in replay_window and 'Height = 420;' in replay_window and
      'MinHeight = 74' in replay_window and
      'root.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });' in replay_window,
      'replay window and status area are enlarged to prevent bottom text clipping')
...
report = root / 'VALIDATION_REPORT_1_13_0_55.txt'
report.write_text(
    'TickLab v1.13.0.55 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
