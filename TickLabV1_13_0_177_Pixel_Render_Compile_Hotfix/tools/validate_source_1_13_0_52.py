...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.52</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.52</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.52</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_52.sln').exists(), 'v52 solution exists')
check(not (root / 'TickLabV1_13_0_51.sln').exists(), 'old v51 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.52', 'VERSION file')
check('TickLabV1_13_0_52.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v52')
check('Tick Replay Vertical Line Repair' in xaml, 'window title')
...
# v1.13.0.52 targeted regression and integrity checks
...
# v1.13.0.52 narrow hotfix checks.
...
# v1.13.0.52 atomic history/live merge checks.
...
# v1.13.0.52 source/display aliasing regression checks.
...
# v1.13.0.52 tick replay and vertical-line regression checks.
...
check('Drag the chart normally away from the line' in replay_window and
      'right-click the line to remove it' in replay_window,
      'replay controls explain non-blocking line interaction')

report = root / 'VALIDATION_REPORT_1_13_0_52.txt'
report.write_text(
    'TickLab v1.13.0.52 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
