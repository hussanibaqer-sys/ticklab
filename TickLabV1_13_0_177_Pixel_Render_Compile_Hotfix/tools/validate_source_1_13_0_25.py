...
for token, label in [
    ('<Version>1.13.0.25</Version>', 'project version'),
    ('<AssemblyVersion>1.13.0.25</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.25</FileVersion>', 'file version'),
]:
    check(token in project, label)
check((ROOT / 'TickLabV1_13_0_25.sln').exists(), 'solution file')
check(text(ROOT / 'VERSION.txt').strip() == '1.13.0.25', 'VERSION.txt')
check('1.13.0.25' in main_xaml, 'main window version title')
...
# V1.13.0.25 vertical-only zoom isolation checks plus prior regressions.
...
print(f'V1.13.0.25 CHECKS PASSED: {passed}')
print(f'V1.13.0.25 CHECKS FAILED: {len(failures)}')
if failures:
    for failure in failures[:150]: print('FAIL:', failure)
    raise SystemExit(1)
print('TickLab v1.13.0.25 vertical-only zoom isolation validation passed.')
