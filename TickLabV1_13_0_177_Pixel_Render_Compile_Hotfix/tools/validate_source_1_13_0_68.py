...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.68</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.68</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.68</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_68.sln').exists(), 'v55 solution exists')
check(not (root / 'TickLabV1_13_0_54.sln').exists(), 'old v54 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.68', 'VERSION file')
check('TickLabV1_13_0_68.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v55')
check('MT5 Trade History + Trading Reports' in xaml, 'window title')
...
# v1.13.0.68 targeted regression and integrity checks
...
# v1.13.0.68 narrow hotfix checks.
...
# v1.13.0.68 atomic history/live merge checks.
...
# v1.13.0.68 source/display aliasing regression checks.
...
# v1.13.0.68 tick replay and vertical-line regression checks.
...
# v1.13.0.68 replay selector and range workflow.
...
# v1.13.0.68 instant replay performance and hidden-live checks.
...
# v1.13.0.68 compact Replay controls and visible speed checks.
...
# v1.13.0.68 replay speed, compact default, selector sharpness/colour and alert colour.
...
# v1.13.0.68 immediate range completion, selector pixels and alert bulk management.
...
# v1.13.0.68 raw Tick chart and alert-contrast checks.
...
check(not (root / 'TickLabV1_13_0_62.sln').exists() and (root / 'TickLabV1_13_0_68.sln').exists(),
...
# v1.13.0.68 Tick chart-type, scale parity, replay layering/performance and turbo checks.
...
# v1.13.0.68 recorder, right-space and ultra-speed checks.
...
# v1.13.0.68 recorder regression: chart-priority capture and fast-player AVI metadata.
...
# v1.13.0.68 dedicated screenshot button and collapsed PANEL startup.
recorder_main = read(app / 'MainWindow.ScreenRecorder.cs')
...
check('TickLabV1_13_0_68.sln' in read(root / 'Clean-Restore-Build.cmd'),
...
for label in ['Starting balance', 'Deposits', 'Withdrawals', 'Ending balance', 'Net trading P/L', 'Win rate', 'Profit factor', 'Maximum drawdown', 'Commission', 'Swap', 'Fees']:
    check(label in trade_report, f'report metric {label}')
for graph in ['Balance / realized-equity curve', 'Drawdown', 'Profit / loss per day', 'Number of trades per day', 'Performance by symbol', 'Monthly profit / loss', 'Trade-by-trade net P/L']:
    check(graph in trade_report, f'report graph {graph}')
check('Zoom −' in trade_report and 'Zoom +' in trade_report and 'Reset' in trade_report and 'ModifierKeys.Control' in trade_report, 'report zoom controls and Ctrl-wheel zoom')
...
report = root / 'VALIDATION_REPORT_1_13_0_68.txt'
report.write_text(
    'TickLab v1.13.0.68 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
