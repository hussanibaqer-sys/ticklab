...
check("<Version>1.13.0.17</Version>" in project, "Project version 1.13.0.17")
check("<AssemblyVersion>1.13.0.17</AssemblyVersion>" in project, "Assembly version 1.13.0.17")
check("<FileVersion>1.13.0.17</FileVersion>" in project, "File version 1.13.0.17")
check((ROOT / "TickLabV1_13_0_17.sln").exists(), "Renamed v1.13.0.17 solution exists")
check((ROOT / "VERSION.txt").read_text().strip() == "1.13.0.17", "VERSION.txt updated")

check("private const double CandleGapPixels = 3.0;" in control, "Compact 3-pixel candle gap restored")
check("CandlePixelGrid grid = CreateCandlePixelGrid(layout);" in renderers, "Shared physical-pixel candle grid")
check("GetSlotPixelSpan(grid, layout.VisibleSlots[visibleIndex])" in renderers, "Shared adjacent slot boundaries")
check("double bodyCenterPixels = (bodyLeftPixels + bodyRightPixels) / 2.0;" in renderers, "Wick derives from final body centre")
check("DrawCandlePixelBody" in renderers and "filled physical-pixel strips" in renderers, "Inside pixel-strip candle borders")
check("drawingContext.DrawRectangle(bodyBrush, borderPen" not in renderers, "Anti-aliased WPF body Pen removed")
check("grid.RawSlotWidthPixels *" in control and "pixelColumn + 0.5" in control, "Compressed candles share one physical column")
...
# Deterministic raster simulation: every detailed adjacent body gap must be
# identical at common chart widths and Windows DPI scales.
for scale in (1.0, 1.25, 1.5, 2.0):
    target_gap = max(1, round(3.0 * scale))
    for plot_width in (801, 997, 1141, 1433, 1919):
        for slot_count in range(70, 300):
            raw_slot = plot_width / slot_count
            if raw_slot < target_gap + 2.0:
                continue
            boundaries = [round(index * raw_slot) for index in range(slot_count + 1)]
            bodies: list[tuple[int, int]] = []
            for index in range(slot_count):
                left = boundaries[index]
                right = boundaries[index + 1]
                actual_gap = min(target_gap, max(0, right - left - 2))
                left_gap = actual_gap // 2
                right_gap = actual_gap - left_gap
                bodies.append((left + left_gap, right - right_gap))
            gaps = [bodies[index + 1][0] - bodies[index][1] for index in range(len(bodies) - 1)]
            check(bool(gaps) and all(gap == target_gap for gap in gaps),
                  f"Uniform raster gap scale={scale} width={plot_width} slots={slot_count}")
...
if failed:
    sys.exit(1)
print("TickLab v1.13.0.17 uniform candle geometry and Step 5 validation passed.")
