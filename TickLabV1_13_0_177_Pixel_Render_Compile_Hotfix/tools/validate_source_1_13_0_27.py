...
for token, label in [
    ('<Version>1.13.0.27</Version>', 'project version'),
    ('<AssemblyVersion>1.13.0.27</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.27</FileVersion>', 'file version'),
]:
    check(token in project, label)
check((ROOT / 'TickLabV1_13_0_27.sln').exists(), 'solution file')
check(text(ROOT / 'VERSION.txt').strip() == '1.13.0.27', 'VERSION.txt')
check('1.13.0.27' in main_xaml, 'main window version title')
...
# V1.13.0.27 right-edge prefetch and sticky-scroll checks plus prior regressions.
...
# V1.13.0.27 exact regression: history prefetch is directional, bounded,
...
# V1.13.0.27 isolated history paging, indicator colour and routing checks.
main_code = text(APP / 'MainWindow.xaml.cs')
routing = text(APP / 'MainWindow.IndicatorRouting.cs')
route_model = text(APP / 'Core/Indicators/IndicatorRouting.cs')
script_appearance = text(APP / 'Core/Scripting/TickScriptIndicatorAppearance.cs')
script_settings = text(APP / 'Windows/TickScriptIndicatorSettingsWindow.cs')
route_window = text(APP / 'Windows/IndicatorRouteWindow.cs')
workspace_preferences = text(APP / 'Settings/WorkspacePreferences.cs')
user_preferences = text(APP / 'Settings/UserPreferences.cs')
settings_store = text(APP / 'Settings/SettingsStore.cs')

for token, label in [
    ('OlderHistoryLoadCancellation', 'per-chart older cancellation'),
    ('NewerHistoryLoadCancellation', 'per-chart newer cancellation'),
    ('OlderHistoryLoadGeneration', 'per-chart older generation'),
    ('NewerHistoryLoadGeneration', 'per-chart newer generation'),
    ('IdentityGeneration', 'chart identity generation'),
    ('ResetContextHistoryPaging', 'history state reset'),
]:
    check(token in contexts, label)
older_start = main_code.find('private async void CandleChart_OlderHistoryRequested')
newer_start = main_code.find('private async void CandleChart_NewerHistoryRequested')
go_home_start = main_code.find('private async void CandleChart_GoToEarliestRequested')
check(older_start >= 0 and newer_start > older_start and go_home_start > newer_start, 'history pager methods found')
older_method = main_code[older_start:newer_start]
newer_method = main_code[newer_start:go_home_start]
check('BeginChartNavigation' not in older_method and 'BeginChartNavigation' not in newer_method,
      'boundary paging does not cancel opposite direction')
check('CompleteNewerHistoryRequest' not in older_method, 'older pager completes only older request')
check('CompleteOlderHistoryRequest' not in newer_method, 'newer pager completes only newer request')
check('stillOwnsRequest' in older_method and 'stillOwnsRequest' in newer_method,
      'obsolete requests cannot release newer pending owner')
check('UpdateChartPagingAvailability(context)' in older_method and 'UpdateChartPagingAvailability(context)' in newer_method,
      'paging availability is per chart')
check('FindChartContext(chart)' in older_method and 'FindChartContext(chart)' in newer_method,
      'history request follows sender chart')

for token in ['LineColor', 'UpperLevelColor', 'LowerLevelColor', 'FillColor', 'FillOpacity', 'LabelColor', 'Visible', 'LinkedGroupId']:
    check(token in script_appearance, f'TickScript appearance {token}')
for token in ['Main series', 'Horizontal levels', 'Fill and labels', 'DrawingColorPickerWindow', 'Fill opacity (0–1)']:
    check(token in script_settings, f'TickScript settings {token}')
for token in ['FillColor', 'FillOpacity', 'LabelColor']:
    check(token in models, f'built-in style {token}')
    check(token in settings, f'built-in settings {token}')
check('Style.FillColor' in builtin_plot and 'Style.FillOpacity' in builtin_plot,
      'built-in pane renders editable fill')
check('Style.FillColor' in text(APP / 'Controls/CandleChartControl.Indicators.cs') and
      'Style.FillOpacity' in text(APP / 'Controls/CandleChartControl.Indicators.cs'),
      'built-in overlay renders editable fill')
check('IndicatorScaleWidth = 48.0' in builtin_plot and 'IndicatorScaleWidth = 48.0' in script_plot,
      'indicator right scales align with price chart')
check('Sync with Price Chart' in indicator_stack, 'sync control label')
check('SyncWithPriceChartChanged' in indicator_stack and 'SyncIndicatorsWithPriceChart' in contexts,
      'sync state is chart-owned')
check('ApplyLocalWheel' in indicator_stack and 'ApplyLocalPan' in indicator_stack,
      'unlinked indicator horizontal viewport')
check('Height = new GridLength(9)' in indicator_stack, 'nine pixel inner divider')

for token in ['Connect', 'Copy', 'Move']:
    check(token in route_model, f'route action {token}')
check('Workspace {WorkspaceId} → Partition {PartitionId} → Chart {PaneId}' in route_model,
      'route target hierarchy')
check('IndicatorRouteWindow' in routing and 'BuildIndicatorRouteTargets' in routing,
      'route target selection')
check('LinkedGroupId' in routing and 'Guid.NewGuid().ToString("N")' in routing,
      'connected indicator groups')
check('AppliedRouteRequested' in indicator_window_cs and 'RouteIndicatorByKey' in management,
      'applied list routing')
check('Destination: Workspace → Partition → Chart' in route_window, 'route dialog hierarchy')

check('AppliedTickScriptIndicators' in user_preferences and 'TickScriptIndicators' in workspace_preferences,
      'custom indicator appearance persistence models')
check('SyncIndicatorsWithPriceChart' in workspace_preferences, 'sync persistence model')
check('TickScriptIndicators = pane.TickScriptIndicators' in settings_store,
      'workspace custom indicator sanitization')
check('CaptureTickScriptIndicatorPreferences' in workspaces and 'RestoreTickScriptIndicatorsForContext' in workspaces,
      'workspace custom indicator save/restore')
check('RefreshAppliedIndicatorsForContext' in main_code and 'LastIndicatorRefreshUtc' in contexts,
      'custom indicator refresh is per chart')
check('History Isolation, Indicator Colours and Routing' in main_xaml, 'release title')
...
print(f'V1.13.0.27 CHECKS PASSED: {passed}')
print(f'V1.13.0.27 CHECKS FAILED: {len(failures)}')
if failures:
    for failure in failures[:150]: print('FAIL:', failure)
    raise SystemExit(1)
print('TickLab v1.13.0.27 history isolation, indicator colours and routing validation passed.')
