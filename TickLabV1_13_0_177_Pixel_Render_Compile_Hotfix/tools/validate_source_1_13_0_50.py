...
# Release identity.
for needle, label in [
    ('<Version>1.13.0.50</Version>', 'version'),
    ('<AssemblyVersion>1.13.0.50</AssemblyVersion>', 'assembly version'),
    ('<FileVersion>1.13.0.50</FileVersion>', 'file version')]:
    check(needle in proj, label)
check((root / 'TickLabV1_13_0_50.sln').exists(), 'v49 solution exists')
check(not (root / 'TickLabV1_13_0_48.sln').exists(), 'old v48 solution name removed')
check(read(root / 'VERSION.txt').strip() == '1.13.0.50', 'VERSION file')
check('TickLabV1_13_0_50.sln' in read(root / 'Clean-Restore-Build.cmd'), 'build script targets v49')
check('Atomic History Live Merge Hotfix' in xaml, 'window title')
...
# v1.13.0.50 targeted regression and integrity checks
...
# v1.13.0.50 narrow hotfix checks.
...
# v1.13.0.50 atomic history/live merge checks.
...
report = root / 'VALIDATION_REPORT_1_13_0_50.txt'
report.write_text(
    'TickLab v1.13.0.50 static validation\n\n'
    f'Passed: {len(passed)}\nFailed: {len(failed)}\n\n' +
